...
@if (session('status'))
        ...
        <div class="card justify-content">
            <div class="card-header">
                <h5 class="title text-center"> Deposit Cash {{ $cash->user->name }} {{ $cash->user->last_name }}</h5><hr>
              </div>
            ...
            <form method="post" action="{{route('balance.storecash')}}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                            <p>Rp. {{$cash->saldo_akhir}},- </p>
                            <input type="hidden" name="id" value="{{$cash['id']}}">
                            <input type="hidden" name="user_id" value="{{$cash['user_id']}}">
                                <input id="saldo_awal" onkeyup="sum();" type="hidden" class="form-control @error('saldo_awal') is-invalid @enderror" name="saldo_awal" value="{{$cash['saldo_akhir']}}">
                                @error('saldo_awal')
